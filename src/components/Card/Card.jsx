...
function Card({ programType, programTitle, programLocation, programTime, programDate }) {
    const programCardTheme = () => {
        if (checkOldDate()) {
            return "old-program"
        }

        if (programType === "برنامج") {
            return "program"
        } else if (programType === "معسكر") {
            return "camp"
        } else if (programType === "لقاء") {
            return "meeting"
        }
    }
    ...
    const checkOldDate = () => {
        ...
        const programDateFormatted = new Date(year, month - 1, day)
        const currentDate = new Date()
        currentDate.setHours(0, 0, 0, 0);

        return programDateFormatted < currentDate ? true : false
    }

    return (
        <div className={`program-card ${programCardTheme()}`}>
            <div className="program-header">
                <p className="program-type">{programType}</p>
                <h3 className="program-title">{programTitle}</h3>
            </div>
            <div className="program-details">
                <div className="program-content">
                    <div className="program-loc-date">
                        <div className="program-location">
                            <p className="program-location-title">{programLocation}</p>
                            <LocationIcon />
                        </div>
                        <div className="program-time-frame">
                            <p className="program-time-frame-title">{programTime}</p>
                            <TimeIcon />
                        </div>
                    </div>
                    <div className="program-date">
                        <button disabled={checkOldDate()} className={`program-details-button ${checkOldDate() ? "disabled" : ""}`}>التفاصيل</button>
                        <div className="program-date-content">
                            <p className="program-date-title"><span>{checkOldDate() ? "بدأ في" : "يبدأ في"}</span> {programDate}</p>
                            <DateIcon />
                        </div>
                    </div>
                </div>
...
const LocationIcon = () => {
    return (
        <svg
            ...
            style={{ enableBackground: 'new 0 0 92.26 122.88' }} // Use double curly braces for inline styles  
            xmlSpace="preserve"
        >
            <g>
                <path
